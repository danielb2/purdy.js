...
internals.isPromise = function (obj) {
    ...
    return !Util.inspect(obj, { depth: 0, customInspect: false }).indexOf('Promise');
};


internals.purdy.prototype.travel = function (object, path, depth) {

    let type = toString.call(object).split(' ')[1].slice(0, -1);

    if (type === 'Object' && internals.isPromise(object)) {
        type = 'Promise';
    }
...
internals.promiseType = function (promise) {

    const opts = { depth: 0, customInspect: false };

    const isRejected = Util.inspect(promise, opts).indexOf('rejected') !== -1;
    if (isRejected) {
        return 'rejected';
    }
    const isPending = Util.inspect(promise, opts).indexOf('pending') !== -1;
    if (isPending) {
        return 'pending';
    }

    const isFulfilled = !Util.inspect(promise, opts).indexOf('Promise { undefined');
    if (isFulfilled) {
        return 'fulfilled';
    }
...
internals.purdy.prototype.Promise = function (promise) {

    const promiseType = internals.promiseType(promise);
    const color = promiseType + 'Promise';

    if (Object.keys(promise).length === 0) {
        return this.colorize('[Promise: ' + promiseType + ']', color);
    }

    const props = this.Object(promise, '', null);
    return props.replace(/^{/, '{ ' + this.colorize('[Promise: ' + promiseType + ']', color) );
};
...
internals.purdy.prototype.Object = internals.purdy.prototype.process = internals.purdy.prototype.global = function (object, path, depth) {
    ...
    if (object.constructor) {
        prefix = ['Object', 'Function', 'Error', 'Promise', ''].indexOf(object.constructor.name) !== -1 ? '' : this.colorize(object.constructor.name, 'prefix') + ' ';
    }
